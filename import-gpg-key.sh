#!/usr/bin/env bash
set -euo pipefail

# Script to import the Digital Asset GPG key for verifying DAML SDK releases
# This can be called by other scripts to ensure the key is properly imported

# Digital Asset's GPG key fingerprint
DAML_GPG_KEY_FINGERPRINT="4911A8DFE1a61941486839D5B9E1236E288F74D4"

# Function to check if the key is already in the keyring
has_gpg_key() {
  gpg --list-keys "$DAML_GPG_KEY_FINGERPRINT" &>/dev/null
}

# Function to import the Digital Asset GPG key
import_gpg_key() {
  echo "Importing Digital Asset GPG key..."
  
  # Import directly from the hardcoded key
  gpg --import <<EOF >/dev/null 2>&1
-----BEGIN PGP PUBLIC KEY BLOCK-----


=srz3
-----END PGP PUBLIC KEY BLOCK-----
EOF
  local retval=$?
  if [ $retval -eq 0 ]; then
    echo "✅ GPG key imported successfully"
    return 0
  else
    echo "❌ Failed to import GPG key"
    return 1
  fi
}

# Main function to ensure the key is available
ensure_gpg_key() {
  if has_gpg_key; then
    return 0
  else
    import_gpg_key
    return $?
  fi
}

# If script is run directly, import the key
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ensure_gpg_key
fi

# Export the functions for use in other scripts
export -f has_gpg_key
export -f import_gpg_key
export -f ensure_gpg_key